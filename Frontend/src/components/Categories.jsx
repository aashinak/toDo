...
function Categories({category,setTodoData}) {
  ...
  useEffect(()=> {
    ...
    const pending = category.todosDetails.filter(todo => todo.isActive)
    const completed = category.todosDetails.filter(todo => !todo.isActive)
  ...
  const handleTodoSubmit = async (data) => {
    setAddTodo(false)
    try {
      const response = await axios.post(`${apiUri}/todo/addTodo`, data, {withCredentials: true})
  ...
  const handleDeleteCategory = async () => {
    try {
      await axios.post(`${apiUri}/todo/deleteCategory`,{categoryId:category._id},{withCredentials: true})
    ...
    <div className="w-full flex flex-col gap-1 select-none">
      ...
      {viewAll ? (
        <div className="w-full p-8 rounded-lg border border-[rgba(255,255,255,0.10)] flex flex-col gap-2">
          ...
          {addTodo ? (
            <div className="w-full">
              <div className=" rounded-lg  border border-[rgba(255,255,255,0.10)] p-1">
                <form onSubmit={handleSubmit(handleTodoSubmit)} className="flex flex-col gap-1 text-white">
                  <input
          ...
          {
            active === "Pending" ?
            pendingTodos.map((todo,index) => <TodoView todoData={todo} key={index}/>)

            :
            completedTodos.map((todo, index) => <TodoView todoData={todo} key={index}/>)
          }
